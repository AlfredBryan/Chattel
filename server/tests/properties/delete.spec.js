...
const url = '/api/v1/delete-property';
...
describe('delete property that belongs to user', function test() {
  before(async () => {
    // create users
  ...
  it('check if users that are not authenticated can delete property', (done) => {
    chai.request(server)
  ...
  it('check if user can delete property with param that is not an integer', (done) => {
    chai.request(server)
  ...
  it('check if user can delete another user single property', (done) => {
    chai.request(server)
      .delete(`${url}/${propertyId}`)
      .set('token', user2.token)
      .end((err, res) => {
  ...
  it('check if user can delete property', (done) => {
    chai.request(server)
      .delete(`${url}/${propertyId}`)
